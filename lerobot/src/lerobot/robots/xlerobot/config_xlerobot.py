...
def xlerobot_cameras_config() -> dict[str, CameraConfig]:
    return {
        # 使用udev符号链接（固定路径）
        # 分辨率可改为 1280x720 或 1920x1080（如果摄像头支持）
        ...
        "head": OpenCVCameraConfig(
            index_or_path="/dev/camera_head", fps=30, width=640, height=480, rotation=Cv2Rotation.NO_ROTATION
        ),
        
        # RealSense深度相机（如果有的话）
        # "head_depth": RealSenseCameraConfig(
...
class XLerobotConfig(RobotConfig):
    ...
    port1: str = "/dev/arm_left"  # 左臂 + 头部 (8个电机)
    port2: str = "/dev/arm_right"  # 右臂 + 底盘 (9个电机)
